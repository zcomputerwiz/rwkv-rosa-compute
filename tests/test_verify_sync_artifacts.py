...
def create_artifact_and_sidecar(
    tmp_path, filename, content=b"hello world", sidecar_fmt="{}  {}", newline="\n", custom_digest=None
):
...
def test_matching_lf_sidecar_verifies(tmp_path):
    ...
    assert verify_module.classify(artifact, sidecar, has_artifact=True, has_sidecar=True) == "verified"
...
def test_tampered_file_reports_mismatch(tmp_path):
    ...
    assert verify_module.classify(artifact_path, sidecar, has_artifact=True, has_sidecar=True) == "MISMATCH"
...
def test_file_no_sidecar_does_not_fail(tmp_path):
    ...
    assert verify_module.classify(str(artifact), sidecar, has_artifact=True, has_sidecar=False) == "no sidecar"


def test_orphaned_sidecar_fails(tmp_path):
    artifact_path, sidecar = create_artifact_and_sidecar(tmp_path, "test.txt")
    assert verify_module.classify(artifact_path, sidecar, has_artifact=False, has_sidecar=True) == "orphaned sidecar"
...
def test_empty_sidecar_reports_malformed(tmp_path):
    ...
    assert verify_module.classify(str(artifact), str(sidecar), has_artifact=True, has_sidecar=True) == "malformed sidecar"
...
def test_non_hex_digest_reports_malformed(tmp_path):
    ...
    assert verify_module.classify(artifact, sidecar, has_artifact=True, has_sidecar=True) == "malformed sidecar"
...
def test_unreadable_reports_unreadable(tmp_path):
    ...
    with patch("builtins.open", mock_open):
        assert verify_module.classify(artifact, sidecar, has_artifact=True, has_sidecar=True) == "unreadable"
