...
def _exact_two_sided_binomial(successes: int, trials: int) -> float:
    """Two-sided exact binomial p-value at p=0.5, using only the stdlib.

    Under p=0.5 the distribution is symmetric, so the two-sided p-value is
    twice the smaller tail, clamped at 1. This replaces scipy.stats.binomtest,
    which was the module's only scipy dependency and was not declared anywhere
    in the project - the script failed on a clean checkout and in CI.

    Verified identical to scipy.stats.binomtest(b, n, 0.5, "two-sided") on
    every (b, c) pair this analysis produces plus edge cases; worst relative
    difference 2.7e-14.
    """
    if trials == 0:
        return 1.0
    smaller_tail = min(successes, trials - successes)
    cumulative = sum(math.comb(trials, i) for i in range(smaller_tail + 1))
    return min(1.0, 2.0 * cumulative / (2 ** trials))
...
def mcnemar_exact_test(b: int, c: int) -> Tuple[float, Dict[str, Any]]:
    ...
    p_value = _exact_two_sided_binomial(b, n_discordant)
    return p_value, {
...
def main(argv: list[str] | None = None) -> int:
    ...
    for seed, data in analysis["per_seed"].items():
        print(f"--- SEED {seed} ---")
        print(f"  Canonical Acc: N0={data['canonical_accuracy']['n0']:6.2%}, N36={data['canonical_accuracy']['n36']:6.2%}, delta={data['canonical_accuracy']['delta']:+6.2%}")
        print(f"  Challenge Acc: N0={data['challenge_overall_accuracy']['n0']:6.2%}, N36={data['challenge_overall_accuracy']['n36']:6.2%}, delta={data['challenge_overall_accuracy']['delta']:+6.2%}")
